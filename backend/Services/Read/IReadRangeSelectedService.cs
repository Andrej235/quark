...
namespace Quark.Services.Read
{
    /// <summary>
    /// Represents a generic interface for getting multiple mapped entities from the database
    /// </summary>
    /// <typeparam name="TEntity">Data model representing the database table</typeparam>
    public interface IReadRangeSelectedService<TEntity>
        where TEntity : class
    {
        /// <summary>
        ...
        /// - <see cref="Result{TValue}.IsSuccess"/> is `true` and <see cref="Result{TValue}.Value"/> contains all entities that fit the <paramref name="criteria"/> mapped according to <paramref name="select"/> <br/>
        ...
        /// </returns>
        Task<Result<IEnumerable<T>>> Get<T>(
